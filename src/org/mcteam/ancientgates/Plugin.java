...
public class Plugin extends JavaPlugin {
	...
	public void onEnable() {
		...
		// Check for updates
		if (Conf.autoUpdate) {
			new Updater(this, 51406, this.getFile(),
...
//			setupSocketComms(sender);
	...
	private void setupSocketComms(CommandSender sender) {
		...
		if (Conf.socketCommsPort > 65535) {
			...
			if (sender != null)
				sender.sendMessage(Conf.colorSystem
						+ "\"socketCommsPort\" is out of range. Try another port.");
			return;
		}
		...
		try {
			serv = new SocketServer(0, Conf.socketCommsPort,
					Conf.socketCommsPass);
		} catch (BindException e) {
			Plugin.log("socketCommsPort already in use. Using generic BungeeCord messaging.");
			if (sender != null)
				sender.sendMessage(Conf.colorSystem + "\"socketCommsPort\" "
						+ Conf.socketCommsPort
						+ " is already in use. Try another port.");
			return;
		}
	...
	private void takedownBungeeCord() {
		...
		Bukkit.getMessenger().unregisterIncomingPluginChannel(this,
				"BungeeCord", pluginMessengerListener);
	...
	// -------------------------------------------- //
	@SuppressWarnings("deprecation")
	public static boolean handleEconManage(CommandSender sender,
			Double requiredCost) {
		if (!(sender instanceof Player))
			return true;
		...
		if (econ == null || !Conf.useEconomy
				|| hasPermManage(sender, "ancientgates.econbypass")
				|| requiredCost == 0.00) {
			return true;
		} else {
			Double balance = econ.getBalance(player.getName());
			if (requiredCost <= balance) {
				EconomyResponse r = econ.withdrawPlayer(player.getName(),
						requiredCost);
				if (r.transactionSuccess()) {
					sender.sendMessage(String.format(
							"You were charged %s and now have %s.",
							econ.format(r.amount), econ.format(r.balance)));
					return true;
				} else {
					sender.sendMessage(String.format("An error occured: %s.",
							r.errorMessage));
					return false;
				}
			} else {
	...
	public String getBaseCommand() {
		...
		Map<String, Map<String, Object>> Commands = this.getDescription()
				.getCommands();
	...
	public void handleCommand(CommandSender sender, List<String> parameters) {
		...
		sender.sendMessage(Conf.colorSystem + "Unknown gate-command \""
				+ commandName + "\". Try " + Conf.colorCommand + "/"
				+ getBaseCommand() + " help");
	}
